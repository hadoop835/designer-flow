...
import {ExtensionElements} from '../../util/bpm'
 ...
 * @param json 
 */
 function parallelGatewaytToXmlJson(json){
  const xmlJson = {};
  Object.entries(json).forEach(([key, value]) => {
    if(key.indexOf('_item_') != -1 || key.indexOf('condition') != -1 || value == undefined){
      ...
      if (typeof value !== 'object') {
        ...
        xmlJson[key] = parallelGatewaytToXmlJson(value);
      }
    } 
  });
  return xmlJson;
 ...
 * @param json 
 */
 function extensionElementsToXmlJson(json){
  const xmlJson = {};
  Object.entries(json).forEach(([key, value]) => {
    if(key.indexOf('event') != -1 || key.indexOf('class') != -1 || key.indexOf('prop') != -1 || value == undefined){
    }else{
      if (typeof value !== 'object') {
        if (key.indexOf('-') === 0) { // 如果本来就是“-”开头的了，那就不处理了。
          xmlJson[key] = value;
        ...
        if(Array.isArray(value)){
          ...
          Object.entries(value).forEach(([k, v]) => {
            if(typeof v === 'object'){
              Object.entries(v).forEach(([kp, vp]) => {
                if(Array.isArray(vp)){
                  let p =new Array();
                  Object.entries(vp).forEach(([_, prop]) => { 
                    p.push(prop);
                  });
                  svalue[kp] = p;
                }else{
          ...
          xmlJson[key] = extensionElementsToXmlJson(value);
        }
      }
    } 
  });
 ...
 * 如果没有“-”表示为子节点
 */
 function seqflowToXmlJson(json) {
  const xmlJson = {};
  Object.entries(json).forEach(([key, value]) => { 
    if (typeof value !== 'object') {
      if (key.indexOf('-') === 0) { // 如果本来就是“-”开头的了，那就不处理了。
...
/**
 * extensionElements: {smart:properties: {…}, smart:executionListener: {…}}
[[Prototype]]: Object
 * 将xmlJson转换为普通的json，在内部使用。
 */
function toImportJson(xmlJson,type) {
  const json = {};
  switch(type){
    ...
    Object.entries(xmlJson).forEach(([key, value]) => {
      if (typeof value === 'string') {
        ...
        Object.entries(value).forEach(([_, ev]) => {
          Object.entries(ev).forEach(([lk, lv]) => {
            if(Array.isArray(lv)){
              var props = new Array();
              Object.entries(lv).forEach(([_, vp]) => {
                var propsMap = {};
                Object.entries(vp).forEach(([k, v]) => {
                  propsMap[k.substring(1)]= v;
                })
                props.push(propsMap);
             })
             json["prop"] = props;
    ...
    Object.entries(xmlJson).forEach(([key, value]) => {
      if (typeof value === 'string') {
        ...
        json[key] = toImportJson(value,type);
      } else {
        json[key] = value;
      }
    })
...
function toNormalJson(xmlJson) {
  ...
  Object.entries(xmlJson).forEach(([key, value]) => {
    if (typeof value === 'string') {
      if(key.indexOf('#') === 0){
        json[key.substring(0,key.indexOf("-"))] = value;
      }else {
        json[key] = value;
      }
    } else if (typeof value === 'object') {
...
function convertLf2ProcessData(bpmnProcessData, data) {
  ...
  data.nodes.forEach((node: NodeConfig) => {
    ...
    if(node.type === BpmnElements.PAGATEWAY){//并行网关，添加属性
      if (node.properties) {
        const properties = parallelGatewaytToXmlJson(node.properties);
        Object.assign(processNode, properties);
      }
    }else if(node.type === BpmnElements.SYSTEM){
      if (node.properties) {
        const properties = extensionElementsToXmlJson(node.properties)
        Object.assign(processNode, properties);
      }
    }
    else{
  ...
  const sequenceFlow = data.edges.map((edge: EdgeConfig) => {
    ...
    if (edge.properties) {
      Object.assign(edgeConfig, edge.properties);
    }
    return edgeConfig;
  });
...
function convertLf2DiagramData(bpmnDiagramData, data) {
  ...
  bpmnDiagramData['bpmndi:BPMNShape'] = data.nodes.map(node => {
    ...
    const shapeConfig = BpmAdapter.shapeConfigMap.get(node.type);
    if (shapeConfig) {
    ...
    Object.keys(process).forEach((key) => {
      if (key.indexOf('bpmn:') === 0) {
        ...
        if (key === BpmnElements.FLOW) {
          ...
          edges = getLfEdges(value, bpmnEdges,nodes);
        } else {
    ...
    if (Array.isArray(nodes)) {
       nodes.forEach(node => {
        if (Array.isArray(edges)) {
          ...
          let count =0;
          edges.forEach(edge => {
            if(node.id == edge.sourceNodeId && (node.type == BpmnElements.PAGATEWAY)){
              let item = {}
              let data = new Array();
              Object.entries(edge.properties).forEach(([key, value]) => {
                Object.entries(value).forEach(([k, v]) => {
                  if(k ==="#cdata"){
                   ...
                   exp["table"] = arr[0];
                   exp["field"] = arr[1];
                   exp["expression"] = arr[2];
                   data.push(exp);
                  }
                })
              })
              item[edge.id+"_item_"+count]=data;
              count++;
              Object.assign(node.properties, item);
            }
          });
        }
      })
...
function getNodeConfig(shapeValue, type, processValue) {
  ...
  const shapeConfig = BpmAdapter.shapeConfigMap.get(type);
  if (shapeConfig) {
  ...
  if (properties) {
    properties = toImportJson(properties,type);
    switch(type){
      case BpmnElements.SYSTEM:
        Object.assign(properties, ExtensionElements(properties))
        break;
    }
  }
  let text;
...
function getLfEdges(value, bpmnEdges,nodes) { 
  const edges = [];
  if (Array.isArray(value)) {
    value.forEach(val => {
      let edgeValue;
      ...
      edges.push(getEdgeConfig(edgeValue, val,nodes));
    });
  } else {
    let edgeValue;
    if (Array.isArray(bpmnEdges)) {
    ...
    edges.push(getEdgeConfig(edgeValue, value,nodes));
  }
  return edges;
}

function getEdgeConfig(edgeValue, processValue,nodes) {
  let text;
...
BpmAdapter.shapeConfigMap.set(BpmnElements.START, {
  width: StartEventConfig.width,
  height: StartEventConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.END, {
  width: EndEventConfig.width,
  height: EndEventConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.GATEWAY, {
  width: ExclusiveGatewayConfig.width,
  height: ExclusiveGatewayConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.SYSTEM, {
  width: ServiceTaskConfig.width,
  height: ServiceTaskConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.USER, {
  width: UserTaskConfig.width,
  height: UserTaskConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.INGATEWAY, {
  width: InclusiveGatewayConfig.width,
  height: InclusiveGatewayConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.PAGATEWAY, {
  width: ParallelGatewayConfig.width,
  height: ParallelGatewayConfig.height,
});
BpmAdapter.shapeConfigMap.set(BpmnElements.RECEIVE, {
  width: ReceiveTaskConfig.width,
  height: ReceiveTaskConfig.height,
});
...
const BpmXmlAdapter = {
  ...
  adapterXmlIn(bpmnData) {
    ...
    return BpmAdapter.adapterIn(json);
  },
  adapterXmlOut(data) {
    const outData = BpmAdapter.adapterOut(data);
